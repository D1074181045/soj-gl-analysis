...
- **陣容配置與分團統計**：把我方玩家分到進攻／機動／防守團（必選三選一），可加保鑣／扛拆／空拆副職（可不選，選則三選一）；戰報多出「分團統計」分頁（各團彙總、對比圖、成員表），玩家詳情可看本團貢獻佔比與跨團輸出對比。配置以玩家名字為鍵，跨場次共用；另可在單場戰報內「調整本場分團」做該場專屬覆寫（優先級：本場調整 > 統一配置，可隨時還原）
